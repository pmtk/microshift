...
func postFailureDebugInfo(ctx context.Context, coreClient *coreclientv1.CoreV1Client) {
	msg := findUnpulledOrFailedImages(ctx, coreClient)
	msg += logPodsAndEvents()

	klog.Infof("DEBUG INFORMATION\n%s", msg)
}

func logPodsAndEvents() string {
...
// findUnpulledOrFailedImages makes an attempt to find images that are still being pulled.
func findUnpulledOrFailedImages(ctx context.Context, coreClient *coreclientv1.CoreV1Client) string {
