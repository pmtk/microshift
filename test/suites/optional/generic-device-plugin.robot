...
Create Test Job
    ...
    ${script}=    Replace String    ${script}    \n    \n${SPACE * 4}
    ${script}=    Catenate    ${SPACE * 4}${script}

    # Add script to ConfigMap
    ${configmap}=    Catenate    ${CONFIGMAP_PREAMBLE}${script}
    Log    ${configmap}
...
Wait Until Device Is Allocatable
    ...
    ${nodeName}=	Remove String	${node}    node/

    Wait Until Keyword Succeeds    60s    5s    Device Should Be Allocatable    ${nodeName}

Device Should Be Allocatable
    [Documentation]    Checks if device device.microshift.io/fakeserial is allocatable
    [Arguments]    ${nodeName}
    ${deviceAmount}=    Oc Get JsonPath
    ...    node
    ...    ${EMPTY}
    ...    ${nodeName}
    ...    .status.allocatable.device\\.microshift\\.io/fakeserial
    Should Be Equal As Integers    ${deviceAmount}    1
